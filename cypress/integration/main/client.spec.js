context('Klinti', function () {
  before(() => {
    cy.login();
  });

  it('klik_na_záložku_Klienti', function () {
    cy.contains('Klienti').click();
    cy.get(
      'body > app-root > div > app-sidebar > aside.z-20.hidden.min-h-screen.h-full.w-64.overflow-y-auto.bg-white.dark\\:bg-gray-800.md\\:block.flex-shrink-0 > div > ul:nth-child(3) > li:nth-child(2) > div > ul > li:nth-child(1) > a'
    ).click();
